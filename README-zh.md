...
> 从[Release](https://github.com/lovitus/processgod/releases)页面可以直接下载程序，启动程序后看到的界面只是一个配置界面，可以在这里配置要守护的进程，启动服务之后可以随时开启或关闭此配置界面
...
配置界面现在提供 CSV 编辑工具，便于备份和批量修改配置。点击状态栏中的 CSV 按钮后，可以在内置表格里直接编辑，也可以导出当前进程配置为 CSV，或导入外部编辑后的 CSV 文件。

CSV 编辑不会替代程序使用的 JSON 主配置文件。保存时，未变化的行会被忽略，发生变化的行会被更新，`Id` 为空或合法但当前不存在的行会作为新增配置追加；CSV 中缺失的旧行不会被删除。如果守护服务正在运行，程序只会把需要生效的新增/变更行通过现有服务命令通道通知服务，不会重启服务。

CSV 导入支持常见中文编码，包括 UTF-8、UTF-16、GB18030、GBK/GB2312 和 Big5。如果文件正被 Excel 或同步软件临时占用，导入器会尽量以共享读取方式打开；读取失败时会显示错误信息，而不是直接关闭程序。

导入命令行参数时，字段中的引号和命令特殊字符会被保留。例如 `/K "ping baidu.com -n 10" & echo done` 这类启动参数导入后不会被改写。
...
**定时计划：** 可选的 5 段 Cron 表达式，用于按计划重启被守护的进程。勾选“仅启动一次”时必须留空。

**执行前停止：** 控制定时计划触发时是否先停止当前正在运行的进程树。
